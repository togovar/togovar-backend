class InspectDisease < ActiveInteraction::Base
  ...
  def execute
    results = Mondo.search(query: { match: { mondo: node || 'MONDO_0000001' } }, sort: 'mondo').results
    ...
    ret = if node.nil? || node == 'MONDO_0000001'
          ...
          elsif (result = results.first).present?
            {
             ...
             .tap { |hash| hash.merge!(leaf: true) if Mondo.count(body: { query: { match: { parent: id } } }).zero? }
          else
            return {}
          end
    ...
    parents = results.map do |r|
      ...
      elsif (x = Mondo.search(query: { match: { mondo: parent } }, size: 100).results.first)
        {
    ...
    children = Mondo.search(query: { match: { parent: node || 'MONDO_0000001' } }, sort: 'mondo', size: 100).results.map do |r|
      {
